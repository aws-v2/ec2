...
func (h *SSHKeyHandler) CreateSSHKey(c *gin.Context) {
	...
	dto.SendSuccess(c, http.StatusCreated, "SSH key created successfully", key)
}
...
func (h *SSHKeyHandler) ListSSHKeys(c *gin.Context) {
	...
	dto.SendSuccess(c, http.StatusOK, "SSH keys retrieved successfully", keys)
}
...
func (h *SSHKeyHandler) DeleteSSHKey(c *gin.Context) {
	...
	if err != nil {
		dto.SendError(c, http.StatusBadRequest, "invalid ssh key id")
		return
	}

	if err := h.service.DeleteSSHKey(id); err != nil {
		if err == domain.ErrSSHKeyNotFound {
			dto.SendError(c, http.StatusNotFound, "ssh key not found")
			return
		}
		dto.SendError(c, http.StatusInternalServerError, err.Error())
		return
	...
	dto.SendSuccess(c, http.StatusOK, "ssh key invalidated successfully", nil)
}
func (h *SSHKeyHandler) DownloadSSHKey(c *gin.Context) {
	...
	if err != nil {
		dto.SendError(c, http.StatusNotFound, "Private key not found or unavailable")
		return
	}
